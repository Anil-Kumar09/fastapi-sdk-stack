...
def get_players(db:Session, skip : int = 0, limt : int = 100,min_last_changed_date: date = None, last_name : str = None, first_name : str = None,):
    ...
    return query.offset(skip).limit(limt).all()

def get_performance(db: Session, skip : int = 0, limit : int = 100, min_last_changed_date : date = None):
    query = db.query(models.Performance)
    if min_last_changed_date:
        query = query.filter(models.Performance.last_changed_date >= min_last_changed_date)
    return query.offset(skip).limit(limit).all()

def get_league(db : Session, skip : int = 0, limit : int = 100, min_last_changed_date : date = None, league_name : str = None):
    ...
    return query.offset(skip).limt(limit).all()

def get_teams(db : Session, skip : int = 0, limt : int = 100, min_last_changed_date : date = None,team_name : str = None,league_id : int = None):
